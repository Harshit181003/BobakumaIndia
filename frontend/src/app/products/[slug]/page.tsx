...
type Detail = {
  ...
  related: { id: number; name: string; slug: string; effectivePricePaise: number; imageUrl: string | null }[];
};

export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const base = process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:4000";
  const res = await fetch(`${base}/api/products/${encodeURIComponent(slug)}`, { next: { revalidate: 15 } });
  if (!res.ok) notFound();
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-4 flex items-baseline gap-3">
            <div className="text-2xl font-bold text-ink-900">₹{(p.effectivePricePaise / 100).toFixed(0)}</div>
            {p.discountPercent > 0 && <div className="text-sm text-ink-900/45 line-through">₹{(p.pricePaise / 100).toFixed(0)}</div>}
          </div>
      ...
      <section className="mt-14">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {data.related.map((r) => (
            <Link key={r.id} href={`/products/${r.slug}`} className="overflow-hidden rounded-3xl border border-white/60 bg-white/60 shadow-sm">
              ...
              <div className="px-3 pb-3 text-sm font-bold">₹{(r.effectivePricePaise / 100).toFixed(0)}</div>
            </Link>
          ))}
        </div>
      </section>
